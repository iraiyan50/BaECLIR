...
MIN_TOKENS = 40
DELAY = 1.0
OUTPUT_FILE = "dhaka_post_articles.json"
...
STRATEGIES = {
    "sitemap": "https://www.dhakapost.com/sitemap.xml",
    "categories": [
...
def extract_article(url):
    """Extract article content from Dhaka Post URL"""
    try:
        stats["attempted"] += 1
        ...
        title_selectors = [
            soup.find("h1", class_=re.compile("title|headline|post-title", re.I)),
            soup.find("h1"),
            soup.find("meta", property="og:title"),
            soup.find("meta", {"name": "twitter:title"}),
            soup.find("div", class_=re.compile("headline", re.I)),
        ]
        ...
        # Try to find article body with common selectors for Dhaka Post
        article_selectors = [
            soup.find("div", class_=re.compile("article-body|story-content|post-content|news-content|detail-content|entry-content", re.I)),
            soup.find("article"),
            soup.find("div", {"itemprop": "articleBody"}),
            soup.find("div", class_=re.compile("content-details|news-details|article-details", re.I)),
            soup.find("div", id=re.compile("article|content", re.I)),
        ]
        ...
        if not date:
            ...
            date_elem = soup.find(class_=re.compile("date|time|published|publish", re.I))
            if date_elem:
        ...
        if not author:
            author_elem = soup.find(class_=re.compile("author|writer|reporter", re.I))
            if author_elem:
        ...
        for url in root.findall(".//{http://www.sitemaps.org/schemas/sitemap/0.9}url"):
            ...
            if loc is not None and loc.text:
                ...
                if all(exclude not in url_text for exclude in ["/page/", "/tag/", "/author/", "/category/", "/archive/"]):
                    if any(cat in url_text for cat in ["politics", "country", "world", "sports", "entertainment", "business", "education", "technology", "lifestyle", "health"]):
                        urls.add(url_text)
                        if len(urls) >= max_urls:
                            break
...
def get_urls_from_category(category_url, max_pages=30):
    ...
    try:
        for page in range(1, max_pages + 1):
            ...
            links = soup.find_all("a", href=re.compile(r"^https://www\.dhakapost\.com/"))
            ...
            for link in links:
                ...
                if href and all(exclude not in href for exclude in ["/page/", "/tag/", "/author/", "/category/", "/archive/"]):
                    ...
                    if any(cat in href for cat in ["politics", "country", "world", "sports", "entertainment", "business", "education", "technology", "lifestyle", "health"]):
...
def main():
    print("="*70)
    print("DHAKA POST ARTICLE SCRAPER")
    print("="*70)
