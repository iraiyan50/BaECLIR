...
MIN_TOKENS = 40
DELAY = 1.0
OUTPUT_FILE = "bd_pratidin_articles.json"
...
STRATEGIES = {
    "sitemap": "https://www.bd-pratidin.com/sitemap.xml",
    "categories": [
        "https://www.bd-pratidin.com/national",
        "https://www.bd-pratidin.com/politics",
        "https://www.bd-pratidin.com/economics",
        "https://www.bd-pratidin.com/international",
        "https://www.bd-pratidin.com/entertainment",
        "https://www.bd-pratidin.com/sports",
        "https://www.bd-pratidin.com/tech",
        "https://www.bd-pratidin.com/campus",
        "https://www.bd-pratidin.com/law-order",
...
def extract_article(url):
    """Extract article content from BD Pratidin URL"""
    try:
        stats["attempted"] += 1
        ...
        title_selectors = [
            soup.find("h1", class_=re.compile("title|headline|post-title|news-title|heading", re.I)),
            soup.find("h1"),
        ...
        # Try to find article body with common selectors for BD Pratidin
        article_selectors = [
            soup.find("div", class_=re.compile("article-body|story-content|post-content|news-content|detail-content|entry-content|news_details|article_content", re.I)),
            soup.find("article"),
            soup.find("div", {"itemprop": "articleBody"}),
            soup.find("div", class_=re.compile("content-details|news-details|article-details|details-content", re.I)),
            soup.find("div", id=re.compile("article|content|news|details", re.I)),
        ]
        ...
        if not date:
            ...
            date_elem = soup.find(class_=re.compile("date|time|published|publish|update", re.I))
            if date_elem:
        ...
        for url in root.findall(".//{http://www.sitemaps.org/schemas/sitemap/0.9}url"):
            ...
            if loc is not None and loc.text:
                ...
                if all(exclude not in url_text for exclude in ["/page/", "/tag/", "/author/", "/category/", "/archive/"]):
                    if any(cat in url_text for cat in ["national", "politics", "economics", "international", "entertainment", "sports", "tech", "campus", "law-order", "literature"]):
                        urls.add(url_text)
                        if len(urls) >= max_urls:
                            break
    ...
    try:
        for page in range(1, max_pages + 1):
            ...
            for link in all_links:
                ...
                # Make sure it's a bd-pratidin.com URL
                if not href:
                    continue
                    
                # Convert relative URLs to absolute
                if href.startswith("/"):
                    href = f"https://www.bd-pratidin.com{href}"
                
                if not href.startswith("https://www.bd-pratidin.com/"):
                    continue
                ...
                if any(cat in href for cat in ["national", "politics", "economics", "international", "entertainment", "sports", "tech", "campus", "law-order", "literature"]):
...
def main():
    print("="*70)
    print("BD PRATIDIN ARTICLE SCRAPER")
    print("="*70)
