...
MIN_TOKENS = 40
DELAY = 1.0
OUTPUT_FILE = "daily_star_articles.jsonl"

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
}

# The Daily Star strategies
STRATEGIES = {
...
def extract_article(url):
    """Extract article from The Daily Star URL"""
    try:
        stats["attempted"] += 1
        ...
        # Title extraction for Daily Star
        title = None
        title_selectors = [
            ("h1", {"class": re.compile("headline|title", re.I)}),
            ("h1", None),
        ...
        # Body extraction for Daily Star structure
        body_text = ""
        
        # Try different content containers used by Daily Star
        content_selectors = [
            {"class": re.compile("article-body|story-body|content-body", re.I)},
    ...
    try:
        ...
        for url in root.findall(".//{http://www.sitemaps.org/schemas/sitemap/0.9}url"):
            ...
            if loc is not None and loc.text:
                ...
                if any(cat in url_text for cat in ["/news/", "/business/", "/sports/", "/opinion/", "/entertainment/", "/lifestyle/"]):
                    if not any(exclude in url_text for exclude in ["/archive/", "/section/", "/page/", "/tag/"]):
                        urls.add(url_text)
...
def get_urls_from_category(category_url, max_pages=20):
    ...
    try:
        for page in range(1, max_pages + 1):
            ...
            # Find article links
            links = soup.find_all("a", href=re.compile(r"^https://www\.thedailystar\.net/.*"))
            
            page_urls = set()
            for link in links:
                href = link.get("href")
                if href and any(cat in href for cat in ["/news/", "/business/", "/sports/", "/opinion/", "/entertainment/"]):
                    # Ensure it's an article, not a category page
                    if href.count('/') >= 5:  # Articles typically have deeper paths
                        page_urls.add(href)
